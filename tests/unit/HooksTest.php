...
class HooksTest extends TestCase {
	...
	public function testLinkAccessCallback() {
		$share = $this->createMock('OCP\Share\IShare');
		$share->method('getShareType')->willReturn(Share::SHARE_TYPE_LINK);
		$event = new GenericEvent(null, ['shareObject' => $share, 'errorCode' => 403]);
		$this->throttleMock->expects($this->once())
			->method('addFailedLinkAccess');
		$this->hooks->linkAccessCallback($event);
	}
	...
	public function testPostLinkAccessCallback() {
		...
		$this->hooks->postLinkAuthCallback($event);
	}

	public function testPreLinkAuthCallback() {
		$share = $this->createMock('OCP\Share\IShare');
		$share->method('getShareType')->willReturn(Share::SHARE_TYPE_LINK);
		$event = new GenericEvent(null, ['shareObject' => $share]);
		$this->throttleMock->expects($this->once())
			->method('applyBruteForcePolicyForLinkShare');
		$this->hooks->preLinkAuthCallback($event);
	}
}
